...
import pytest
from new_frontera.contrib.backends.hbase import HBaseState, HBaseMetadata, HBaseQueue
...
class TestHBaseBackend(object):
    ...
    def test_metadata(self):
        ...
        assert set(
            [to_native_str(data[b"m:url"], "utf-8") for _, data in table.scan()]
    ...
    def test_queue_with_delay(self):
        ...
        with mock.patch("new_frontera.contrib.backends.hbase.time") as mocked_time:
    ...
    def test_drop_all_tables_when_table_name_is_str(self):
        ...
        assert set(tables) == set(
            [b"metadata", b"queue", b"states"]
        )  # Failure of test itself
        try:
            HBaseQueue(
