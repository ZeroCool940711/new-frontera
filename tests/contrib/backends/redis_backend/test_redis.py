...
from new_frontera.contrib.backends.redis_backend import (
    FIELD_DOMAIN_FINGERPRINT,
    FIELD_ERROR,
    FIELD_STATE,
)
from new_frontera.contrib.backends.redis_backend import FIELD_STATUS_CODE, FIELD_URL
from new_frontera.contrib.backends.redis_backend import (
    RedisBackend,
    RedisMetadata,
    RedisQueue,
...
class Request:
    def __init__(self, fingerprint, crawl_at, url, domain=None):
        self.meta = {b"crawl_at": crawl_at, b"fingerprint": fingerprint}
        if domain:
            self.meta[b"domain"] = {
                b"name": domain,
                b"fingerprint": "d_{}".format(fingerprint),
            }
        self.url = url
        self.method = "https"
        self.headers = {}
...
class RedisQueueTest(TestCase):
    ...
    def setup_subject(partitions):
        settings = Settings(module="new_frontera.settings.default_settings")
        return RedisQueue(
            WorkerFrontierManager.from_settings(settings), get_pool(), partitions, True
        )
    ...
    def test_scheduling_past_1part_5(self):
        ...
        batch = [
            ...
            (
                ...
                0.5,
                Request(
                    "3", int(time()) - 10, "https://www.hellan.me/", domain="hellan.me"
                ),
                True,
            ),
        ]
        ...
        requests = subject.get_next_requests(
            5, 0, min_hosts=1, min_requests=1, max_requests_per_host=5
        )
    ...
    def test_scheduling_past_1part_1(self):
        ...
        self.assertEqual(3, subject.count())
        requests = subject.get_next_requests(
            1, 0, min_hosts=1, min_requests=1, max_requests_per_host=5
        )
    ...
    def test_scheduling_past_1part_2(self):
        ...
        self.assertEqual(3, subject.count())
        requests = subject.get_next_requests(
            2, 0, min_hosts=1, min_requests=1, max_requests_per_host=5
        )
    ...
    def test_scheduling_past_2part_5(self):
        ...
        requests = subject.get_next_requests(
            5, partition_id=0, min_hosts=1, min_requests=1, max_requests_per_host=5
        )
        self.assertEqual(2, len(requests))
        urls = [request.url for request in requests]
        self.assertTrue("https://www.knuthellan.com/" in urls)
        ...
        requests = subject.get_next_requests(
            5, partition_id=1, min_hosts=1, min_requests=1, max_requests_per_host=5
        )
        self.assertEqual(1, len(requests))
        urls = [request.url for request in requests]
        self.assertTrue("https://www.hellan.me/" in urls)
    ...
    def test_scheduling_past_2part_2(self):
        ...
        self.assertEqual(3, subject.count())
        requests = subject.get_next_requests(
            2, partition_id=0, min_hosts=1, min_requests=1, max_requests_per_host=5
        )
        self.assertEqual(2, len(requests))
        urls = [request.url for request in requests]
        self.assertTrue("https://www.knuthellan.com/" in urls)
        ...
        requests = subject.get_next_requests(
            2, partition_id=1, min_hosts=1, min_requests=1, max_requests_per_host=5
        )
        self.assertEqual(1, len(requests))
        urls = [request.url for request in requests]
        self.assertTrue("https://www.hellan.me/" in urls)
    ...
    def test_scheduling_past_2part_1(self):
        ...
        self.assertEqual(3, subject.count())
        requests = subject.get_next_requests(
            1, partition_id=0, min_hosts=1, min_requests=1, max_requests_per_host=5
        )
        self.assertEqual(1, len(requests))
        urls = [request.url for request in requests]
        self.assertTrue("https://www.knuthellan.com/" in urls)
        ...
        requests = subject.get_next_requests(
            1, partition_id=1, min_hosts=1, min_requests=1, max_requests_per_host=5
        )
        self.assertEqual(1, len(requests))
        urls = [request.url for request in requests]
        self.assertTrue("https://www.hellan.me/" in urls)
    ...
    def test_get_next_requests_min_hosts(self):
        ...
        self.assertEqual(3, subject.count())
        requests = subject.get_next_requests(
            1, partition_id=0, min_hosts=2, min_requests=1, max_requests_per_host=5
        )
        self.assertEqual(2, len(requests))
        urls = [request.url for request in requests]
        self.assertTrue("https://www.knuthellan.com/" in urls)
    ...
    def test_get_next_requests_min_hosts_high_number(self):
        ...
        self.assertEqual(3, subject.count())
        requests = subject.get_next_requests(
            1, partition_id=0, min_hosts=5, min_requests=1, max_requests_per_host=5
        )
        self.assertEqual(2, len(requests))
        urls = [request.url for request in requests]
        self.assertTrue("https://www.knuthellan.com/" in urls)
    ...
    def test_get_next_requests_max_requests_2(self):
        ...
        self.assertEqual(5, subject.count())
        requests = subject.get_next_requests(
            5, partition_id=0, min_hosts=1, min_requests=1, max_requests_per_host=2
        )
...
class RedisStateTest(TestCase):
    def test_update_cache(self):
        ...
        r1.meta[b"state"] = b"a"
        r2 = Request(
            "2", int(time()) - 10, "https://www.khellan.com/", domain="khellan.com"
        )
        r2.meta[b"state"] = b"b"
        r3 = Request(
            "3", int(time()) - 10, "https://www.hellan.me/", domain="hellan.me"
    ...
    def test_flush_force(self):
        ...
        r1.meta[b"state"] = b"d"
        r2 = Request(
            "5", int(time()) - 10, "https://www.khellan.com/", domain="khellan.com"
        )
        r2.meta[b"state"] = b"e"
        r3 = Request(
            "6", int(time()) - 10, "https://www.hellan.me/", domain="hellan.me"
        )
        r3.meta[b"state"] = b"f"
        batch = [r1, r2, r3]
        subject.update_cache(batch)
    ...
    def test_fetch(self):
        ...
        r1.meta[b"state"] = b"g"
        r2 = Request(
            "8", int(time()) - 10, "https://www.khellan.com/", domain="khellan.com"
        )
        r2.meta[b"state"] = b"h"
        batch = [r1, r2]
        subject.update_cache(batch)
        subject.flush(True)
        r3 = Request(
            "9", int(time()) - 10, "https://www.hellan.me/", domain="hellan.me"
        )
        r3.meta[b"state"] = b"i"
...
class RedisMetadataTest(TestCase):
    def test_add_seeds(self):
        ...
        r2 = Request(
            "md2", int(time()) - 10, "https://www.khellan.com/", domain="khellan.com"
        )
        r3 = Request(
            "md3", int(time()) - 10, "https://www.hellan.me/", domain="hellan.me"
        )
        seeds = [r1, r2, r3]
        subject.add_seeds(seeds)
        connection = StrictRedis(connection_pool=pool)
        self.assertEqual(
            b"https://www.knuthellan.com/", connection.hmget("md1", FIELD_URL)[0]
        )
        self.assertEqual(b"d_md1", connection.hmget("md1", FIELD_DOMAIN_FINGERPRINT)[0])
        self.assertEqual(
            b"https://www.khellan.com/", connection.hmget("md2", FIELD_URL)[0]
        )
        self.assertEqual(b"d_md2", connection.hmget("md2", FIELD_DOMAIN_FINGERPRINT)[0])
        self.assertEqual(
            b"https://www.hellan.me/", connection.hmget("md3", FIELD_URL)[0]
        )
    ...
    def test_links_extracted(self):
        ...
        l2 = Request(
            "l2", int(time()) - 10, "https://www.khellan.com/", domain="khellan.com"
        )
        l3 = Request(
            "l3", int(time()) - 10, "https://www.hellan.me/", domain="hellan.me"
        )
        links = [l1, l2, l3]
        subject.links_extracted(None, links)
        connection = StrictRedis(connection_pool=pool)
        self.assertEqual(
            b"https://www.knuthellan.com/", connection.hmget("l1", FIELD_URL)[0]
        )
        self.assertEqual(b"d_l1", connection.hmget("l1", FIELD_DOMAIN_FINGERPRINT)[0])
        self.assertEqual(
            b"https://www.khellan.com/", connection.hmget("l2", FIELD_URL)[0]
        )
        self.assertEqual(b"d_l2", connection.hmget("l2", FIELD_DOMAIN_FINGERPRINT)[0])
        self.assertEqual(
            b"https://www.hellan.me/", connection.hmget("l3", FIELD_URL)[0]
        )
...
class RedisBackendTest(TestCase):
    ...
    def setup_subject(partitions):
        ...
        return RedisBackend.db_worker(
            WorkerFrontierManager.from_settings(settings, db_worker=True)
        )

    def test_get_next_request(self):
        subject = self.setup_subject(2)
        requests = subject.get_next_requests(
            max_next_requests=10, partitions=["0", "1"]
        )
        self.assertEqual(0, len(requests))
    ...
    def test_close_manager(self):
        settings = Settings(module="new_frontera.settings.default_settings")
        settings.set(
            "BACKEND", "new_frontera.contrib.backends.redis_backend.RedisBackend"
        )
